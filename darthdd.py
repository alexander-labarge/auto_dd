...
def partitionProbe(dest_drive):
    """Creates a new GPT partition table and refreshes the kernel's view of the drive."""
    try:
        subprocess.run(["partprobe", dest_drive], check=True)
    except subprocess.CalledProcessError as e:
        print(f"Error in partition probing {dest_drive}: {e}")
        return False
    return True
...
def run_dd(source_drive, dest_drive, block_size, benchmark_size):
    ...
    seek_value = 4 * 1024 * 1024 // block_size  # 4MB offset in terms of blocks
...
dd if={source_drive} of={dest_drive} bs={block_size} seek=8192 status=progress
"""
